...
function cd(args) {
  var resp = "bash: cd: {dir} Permission refusée !";
  var dir = "/:";

  if (args.length > 0) {
    var isDirectory = args[0].match(/\.\w+/g) == null;
    if (!isDirectory) {
      resp = "bash: cd: " + args[0] + ": Ce n'est pas un répertoire !";
    } else {
      dir = args[0] + ":";
    }
  }
...
function creds() {
  terminalPrint("Merci a github.com/Nathack-dev pour l'idée !")
}

function phenix() {
  terminalPrint("=> Voir phenix.txt")
